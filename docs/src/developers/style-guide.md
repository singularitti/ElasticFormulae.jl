...
!!! info
    The style guide is always a work in progress, and not all ElasticFormulae code
    follows the rules. When modifying ElasticFormulae, please fix the style violations
...
ElasticFormulae uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/ElasticFormulae.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the ElasticFormulae directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to ElasticFormulae have
