...
[ElasticFormulae](https://github.com/singularitti/ElasticFormulae.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("ElasticFormulae")
   ```

3. Run

   ```julia-repl
   julia> using ElasticFormulae
   ```
...
version of ElasticFormulae, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/ElasticFormulae.jl"
```

in the second step above.

## Update ElasticFormulae
...
our [GitHub repository](https://github.com/singularitti/ElasticFormulae.jl)
for new releases.
Once we release a new version, you can update ElasticFormulae by typing

```@repl
using Pkg
Pkg.update("ElasticFormulae")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall ElasticFormulae
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("ElasticFormulae")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall ElasticFormulae.
